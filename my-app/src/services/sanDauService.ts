...
export const sanDauService = {
  /** Lấy danh sách sân đấu có phân trang, tìm kiếm và lọc theo cụm sân, trạng thái */
  ...
  /** Lấy tất cả sân đấu (có thể lọc theo cụm sân) */
  getAll: async (cumSanId?: number) => {
    const res = await api.get<SanDau[]>('/api/sandau', { params: { cumSanId } });
    return res.data;
  },
