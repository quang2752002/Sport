...
namespace Dms.Application.Services
{
    ...
    public class SanDauService : ISanDauService
    {
        ...
        public async Task<IEnumerable<SanDauDto>> GetAllAsync(int? cumSanId = null)
        {
            var paged = await _unitOfWork.SanDaus.GetPagedAsync(
                pageIndex: 1,
                pageSize: 1000,
                predicate: s => s.IsDeleted != true && s.TrangThai && (!cumSanId.HasValue || s.CumSanId == cumSanId.Value),
        ...
        {
            var item = await _unitOfWork.SanDaus.GetByIdAsync(id);
            if (item == null || item.IsDeleted == true) return null;
        ...
        public async Task<SanDauDto> CreateAsync(CreateUpdateSanDauDto dto, string? createdBy = null)
        {
            ...
            return _mapper.Map<SanDauDto>(entity);
        }
